...
class CurriculumManager {
  constructor() {
    ...
    this.SWcurrName3 = [
      ...
      "Graduation ProjectsⅠ",
      "Principles of Management",
      "Computer Graphics",
      "Computer Architecture",
      "P-Practical Project",
      "Graduation ProjectsⅡ",
    ]; // 9 + 2
    this.SWcurrName4 = [
      "Computer Vision",
      "Technology Management",
      "You Make Course",
      "Graduation ProjectsⅢ",
      "Data Management R&D Lab",
      "Chatbot R&D Lab",
      "system Architecture R&D Lab",
...
class Game {
  ...
  initRound(round) {
    ...
    if (round === 1) {
      ...
      camera.lookAt(
        new THREE.Vector3(defaultDestX, defaultDestY, defaultDestZ)
      );
  ...
  document.addEventListener("keydown", function (ev) {
    ...
    if (paused) {
      if (inputKey === spacebar) {
        ...
        count++;
        if (count == 1) {
          //처음 시작하거나, 스트링 나오고 설명 나오게 하려는 부분
          console.log("Count!" + count);
          if (roundNumber == null) {
            roundNumber = 1;
...
function createCurriculums(position, probability, minScale, maxScale) {
  ...
  currManager.currWordDict[currManager.index] =
    SWcurrNameList[currManager.index];
